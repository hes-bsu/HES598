...
### Below here, things are still sketchy and not working
...
e<- extent(-107.1678, -106.6813, 38.63501, 39.06351)
p<- as(e, "SpatialPolygons")
crs(p) = "+proj=longlat +datum=WGS84"

pp<- spTransform(p, CRS(data_proj))

#r<- raster(test, ext=e, crs=data_proj, resolution=30, ...)
r<- raster(pp)
...
plot(starfm_binary, col = rev(cm.colors(40)), main = "Snow/No-Snow from Model")
plot(ER_proj, border = "black", add = TRUE)
plot(test, col = rev(cm.colors(40)), main = "Snow/No-Snow from Random Raster")
#plot(ER_proj, border = "black", add = TRUE)
