...
#r_all<- stack()
r_all<- array(NA, dim=c(1590, 1805, n))

for (i in 1:length(rand)) {
  rand_layer<- rand[[i]]
  dim(rand_layer)<- c(1590, 1805)
  r_all[[i]] = rand_layer
  #r_all[[i]]<- raster(rand_layer, crs=data_proj)
  #extent(r_all)<- e
  #res(r_all)<- 0.0002694946
  #rand_all[[i]]<- r_all
}
...
plot(starfm_binary, col = rev(cm.colors(40)), main = "Snow/No-Snow from Model")
plot(ER_proj, border = "black", add = TRUE)
plot(r, col = rev(cm.colors(40)), main = "Snow/No-Snow from Random Raster")
plot(ER_proj, border = "black", add = TRUE)
