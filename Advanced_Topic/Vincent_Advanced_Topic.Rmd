...
To validate the results of the model, a single Landsat image is excluded from the model input data, and then layer is used to compare the synthetic image produced from the model from that same date to see how accurate the model's prediction was. One way in which we quantify the performance of the model is by binary classification. We use the common NDSI threshold of 0.4 (Hall et al., 1995) to decide whether a pixel is classified as snow or no snow. Any value at 0.4 or above is given the label of "snow" (the positive classification), and any value below is given the label of "no snow" (the negative classification). 

Once each pixel is placed into one of these categories, a confusion matrix of true positives, true negatives, false positives, and false negatives is generated. From these values we then calculate the following metrics to answer the questions:

accuracy - What fraction of pixels (snow and no-snow) does the model classify correctly?
precision - What fraction of classified "snow" was actually snow?
recall - What fraction of actual snow (the positive class) did the model catch? How good is the model at predicting snow? 
F-score - What is the accuracy of the model regarding its performance on identifying the positive (snow) class?

These last 3 metrics are used because, as our dates of analysis move further into the spring, snow becomes more scarce in the landscape and the negative, "no-snow", class becomes the majority classifier, which can inflate the accuracy values for even an unskilled model.

In addition to the calculation of the F-score for the model output, it is also important to quantify how well the model is performing against a random distribution of snowcover (Walters, 2013). This project proposes to do just that. Below is a draft outline of the steps I hope to accomplish in this analysis:
...
The code for this section is based on a script originally written by Faye Peters (Peters, 2016), modified by Megan Gallagher and Jake Graham for use in Megan's M.S. thesis (Gallagher, 2018), and then finally further modified by myself for my own research purposes.
...
landsat_apr25<- raster("./Vincent_Data/Landsat_testday_042516.tif") #the validation data
starfm_apr25<- raster("./Vincent_Data/STARFM_testday_042516.tif") #the model data
ER<- readOGR("./Vincent_Data/EastRiver_Project.shp") #shapefile of the study watershed for reference
...
## The lines on the top of the bars in the right boxplot (the randomized results) are the standard deviations of each metric. These values ended up being so small that they are almost not worth putting on the graph (and aren't even able to be seen for the f-score), but in the interest of being thorough, I decided to go ahead and include them anyways. 

```

## Reflect

I found this exercise to be beneficial in several ways. First, I was able to convert some of the functions I had been using in my research up to this point, specifically the creation of the matrices of 0s and 1s for classification and the generation of the confusion matrices for model vs validation data, from Matlab scripts that I had largely pieced together by hand into a single, much more efficient R script. Second, I am pleased that I was able to accomplish the goal I had for this project. I was able to randomize a single matrix multiple times, calculate classification statistics for each one, and display the results of those statistics against the model data. On top of all of that, the final numbers showed what I had hoped they would, that my model does do better at predicting snow than the random distribution. 

I feel that my general skill and comfort level with coding in R has improved quite a bit. The part of the code I struggled the most with was converting a list of matrices back into a list of rasters with the correct extent, resolution, and projection. In working on this problem, I gained a much better understanding of how R deals with raster data, and what is involved with getting it ready to be used for analysis. 

I was a little surprised that the variance in the metrics calculated from the randomized rasters was so small. I am pretty confident that by using the sample() function, the rasters I analyzed were legitimately randomized. I need to do some further looking into this, however, so see if this is common in other studies that have used this same analysis. 

My next steps will be to expand this process to be able to analyze more than one date of results from my model. I think my biggest challenge will be to move this code onto a platform like R2, where I can process bigger files and more data than what my local machine can handle. 

## References

Gallagher, M. (2018). "Utilizing satellite fusion methods to assess vegetation phenology in a semi-arid ecosystem." [Boise, Idaho] : Boise   State University, 2018. BSU Albertsons Library ebook -- Online LD458.8.G5 G355 2018eb
  http://catalog.boisestate.edu/cgi-bin/Pwebrecon.cgi?LANGUAGE=English&DB=local&BBID=1295025
...
Peters, F. (2016). "Analyzing the phenologic dynamics of kudzu (Pueraria montana) infestations using remote sensing and the normalized      difference vegetation index." (2016). Electronic Theses and Dissertations. Paper 2437. https://doi.org/10.18297/etd/2437

Walters, R. (2013). "Transfer of snow information across the macro-to-hillslope-scale gap using a physiographic downscaling approach:      implications for hydrologic modeling in semiarid, seasonally snow-dominated watersheds." (dissertation). Boise State University.
