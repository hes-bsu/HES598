...
landsat_apr25<- raster("./Data/Landsat_testday_042516.tif")
starfm_apr25<- raster("./Data/STARFM_testday_042516.tif")
...
## Project a single random matrix 
test<- rand[[1]]
...
## Convert the list of the random samples (currently in vector format) into matrices with the correct dimensions
...
## Convert the matrices of the randomly sampled raster values into raster format and project to match the actual data
r_all<- vector("list", n)
...
plot(starfm_binary, col = rev(cm.colors(40)), main = "Snow/No-Snow from Model")
plot(ER_proj, border = "black", add = TRUE)
plot(r_all[[10]], col = rev(cm.colors(40)), main = "Snow/No-Snow from Random Raster")
plot(ER_proj, border = "black", add = TRUE)
...
for (i in 1:length(r_all_vec)) { 
 vec_to_fac<- r_all_vec[[i]]
