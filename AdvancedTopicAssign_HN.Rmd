...
# Final Product
Originally, I attempted to create plots to compare sample to population and the effects of post-stratification on preference for sagebrush, but kept reaching an error in gzfile(file,wb). 

When I later attempted this after fixing some issues with package dependencies, I was able to produce the plots I was hoping to produce.
```{r Final Product for MRP, warning=FALSE}
...
educ_popn <- poststrat%>%
  group_by(educ)%>%
  summarize(Num=sum(N))%>%
  mutate(PROP=Num/sum(Num),TYPE='Popn',VAR='Eucation',CAT=educ)%>%
  ungroup()
educ_data <- sample%>%
  group_by(educ)%>%
  summarise(Num=n())%>%
...
load("mrp-files/plot_data.rda") # created above
...
levels(summary_by_poststrat_var$variable) <- list('Education'='educ','Envrionmental Attitude'='env_att','Income'='income','Male'='male')
...
After additional trial-and-error, in addition to running into issues from a software update breaking my RStudio and Github connection a few times, I was ...
