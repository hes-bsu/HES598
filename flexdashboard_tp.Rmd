...
This dashboard was created to help explore two different ways of displaying COVID-19 data. The critical step for this analysis is tidying the data into a usable format that the mapping packages can understand. The next crucial step was finding an engaging way to display the data. I chose two different packages to do this an in effort to compare process and outcomes of both packages. In order to assess the efficiency of each mapping process, I used microbenchmark to measure the time it takes to create each map. Overally, the highcharter map has a faster processing time then leaflet map. 
