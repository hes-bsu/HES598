...
- `fasterize` is `r floor(median(raster.time.mb$time)/median(faster.time.mb$time))` faster than `raster`
...
rasterize.prof <- profvis(expr = {
  ...
  income.tct.sp <- get_acs(geography = "tract", 
              variables = "B19013_001",
              state = "ID", 
              year = 2018, geometry = TRUE) %>% as(., "Spatial")
  elev <- raster("./data/id_elev.tif")
  #process into raster
...
elev.ext1 <- elev.crop.vx$extract(sp=idaho.inc.crop1, 
                                  fun = function(x) 
                                    mean(x, na.rm = TRUE))
...
elev.ext2 <- elev.crop.vx$extract(sp=idaho.inc.crop2, 
                                  fun = function(x) 
                                    mean(x, na.rm = TRUE))
...
small_extract_rst <- function(rstr,counter, crprstr,vec){
  ...
  rst.crp <- crop(rstr,flt.rst)
  vec.crp <- crop(vec, rst.crp)
  rst.extract <-extract(rst.crp, vec.crp, fun=mean)
}
```

## Compare the timing

- `velox` is `r floor(median(rs.timing$time)/median(vx.timing$time))` faster

```{r funtime, cache=TRUE}
vx.timing <- microbenchmark(lapply(values(raster.boxes), 
                   function(x) small_extract(rstr=elev.p,
                                             counter=x,
                                          crprstr = raster.boxes,
                                          income.tct.sp)), 
                   times = 20)
rs.timing <- microbenchmark(lapply(values(raster.boxes), 
                   function(x) small_extract_rst(rstr=elev.p,
                                             counter=x,
                                          crprstr = raster.boxes,
                                          income.tct.sp)), 
                   times = 20)
