...
idaho.sf <- sf::st_read("./data/idaho_cty.shp") # double colon allows to use the library you've already installed
sf::st_crs(idaho.sf) #unprojected data set (no units)
...
nad83 <- "+init=epsg:4269" #only area changes here
idaho.cent <- "+proj=tmerc +lat_0=41.66666666666666 +lon_0=-114 +k=0.9999473679999999 +x_0=500000.0001016001 +y_0=0 +ellps=GRS80 +datum=NAD83 +to_meter=0.3048006096012192 +no_defs " #area and direction change for this one

idaho.83 <- spTransform(idaho, CRS(nad83)) #spTransform(idaho, CRS(nad83)) <- idaho.83 => this will only add information but won't reproject it (won't change geometry)
idaho.c <- spTransform(idaho, CRS(idaho.cent))

#are they different? YES
identicalCRS(idaho.83, idaho.c)

#reproject idaho.cent => reprojecting idaho.83 to match idaho.c (instead of CRS use proj4 - cleaner way to do this)
idaho.reproj <- spTransform(idaho.83, proj4string(idaho.c))

#are they different? NO => should because it's reprojected
...
st_crs(idaho.sf.83) == st_crs(idaho.sf.c) #checking to see that both are equal (==)

##reproject using sf syntax
idaho.sf.reproj <- st_transform(idaho.sf.83, st_crs(idaho.sf.c)) #using crs instead of proj4
...
idaho.join <- tigris::geo_join(idaho.83, data_frame = idaho.nonprof, by_sp = "GEOID", by_df="FIPS", how="left") #spatial data, tabular data, key => telling what columns to match LJoin
...
idaho.join.sf <- idaho.sf.83 %>% #take data set and move it down the line without saving it 
  left_join(., idaho.nonprof, by = c("GEOID" = "FIPS")) #thing I want from the first is equivalant to the second - Joining is more specific than 'tigris' & 'sp'
head(idaho.join.sf)
str(idaho.join.sf)
...
idaho.ext.mn <- raster::extract(elev, idaho.proj, fun=mean) #list with the same length of polygon within list all pixels with all info
```
## Extracting Data from Rasters (cont'd)
```{r ext2, eval=FALSE}
#using velox
elev.vx <- velox(elev)
elev.ext.vx <- elev.vx$extract(sp = idaho.proj) #takes everything in memory at once and processes it in c++
#with sf
