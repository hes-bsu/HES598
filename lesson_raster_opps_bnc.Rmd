...
plotRGB(landsatFCC, axes=TRUE, stretch="lin", main="Landsat False Color Composite") #used to distinguish between features
par(mfrow =c(1,1))
```
...
#Center data => make sure that units is brought to a common scale
#Center data => distance from the mean
```
...
```{r}
veg <- reclassify(NDVI,cbind(-Inf, 0.4,NA))#(from, to, becomes)
...
```{r}
ndvi.percentile <- quantile(NDVI, probs = seq(0, 1.0, 0.1), names=FALSE)
