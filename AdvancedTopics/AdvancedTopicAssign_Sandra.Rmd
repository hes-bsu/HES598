...
library(raster)# I used it to work with a raster file
library(rasterVis)
...
### Then I grab the name of the ID_pixel from the pixels_spdf
...
################### reproject raster from lat long into UTM
...
writeRaster(elev_utm, filename="C:/Users/Sandra/Desktop/R files/Stats/RMarkdownAssignment_Sandra/datatemp/original/elev_utm.tif", overwrite=TRUE)
...
myCol <- terrain.colors(6)
plot(elev_utm, col=myCol)
points(df$x, df$y, main="elev", xlab="East", ylab="North", cex = 0.5)
...
#coord=coordnt
#coord <- st_set_geometry(coord,NULL)
#colnames(coord)

#myvars <- c("East", "North")
#coord_new <- coord[myvars]
#colnames(coord_new)

#as.matrix(sapply(coord_new, as.numeric))  

#id <- c("ID_veg", "ID_pixel")
#coord_id <- coord[id]

#as.matrix(sapply(coord_id, as.numeric))  
...
# create a transition matrix from the original
...
## write.table(hd,file="hd.csv", append=FALSE, sep= ",", row.names = FALSE, col.names=TRUE)
...
# coords <- st_as_sf(coords, coords = c("longitude", "latitude"),                    crs = 4326, stringsAsFactors = FALSE) %>%          st_transform(crs = 32619)

coordnt

dist2 <- costDistance(conductance,
                      fromCoords = as(as_Spatial(coordnt[1,]), "SpatialPoints"),
                      toCoords = as(as_Spatial(coordnt), "SpatialPoints"))


dist2

#names(dist2) <- coordnt$ID_pixel
#dist2

#dist2_df <- data.frame(ID = names(dist2), values = as.vector(dist2))
#head(dist2_df)


#dist2_fn <- dist2_df %>% left_join(. , coordnt[,c("ID_pixel", "ID_veg")], by = c("ID" = "ID_veg"))
#dist2_fn

coord_df <- st_drop_geometry(coordnt)
coord_df

dist2_fn <- dist2_df %>% left_join(. , coord_df[,c("ID_pixel", "ID_veg")], by = c("ID" = "ID_pixel"))
head(dist2_fn)
...
hex_elev_utm_shape <- st_make_grid(elev_utm_shape, cellsize = 50, square = FALSE)
hex_elev_utm_shape
...
edgelist <- as.matrix(as.data.frame(st_intersects(hex_elev_utm_shape)))
...
hex_pixels <- as.numeric(st_intersects(coordnt, hex_elev_utm_shape))
hex_pixels

hex_pixels <- unique(as.numeric(st_intersects(coordnt, hex_elev_utm_shape)))
hex_pixels

dist3 <- distances(gr, hex_pixels[1], hex_pixels[-1])

dist3_df <- data.frame(dist3, row.names = "Distance from init:")


coordnt$ID_veg
colnames(dist3_df)<- c(coordnt$ID_veg)
