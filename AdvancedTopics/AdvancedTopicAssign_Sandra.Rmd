...
#raster_hd <- geoCorrection(hd, type = "c")

#rasValue=extract(hd, pointCoordinates)
...
slope <- geoCorrection(hd, type="c", scl=FALSE)
...
### I need to conver raster to polygon###
...
## Let’s create another grid, but this time, let’s make it hexagonal

hex_elev_umt <- st_make_grid(elev_utm, cellsize = 250, square = FALSE)
hex_elev_umt
...
edgelist <- as.matrix(as.data.frame(st_intersects(hex_elev_umt)))
  # yes, the as.matrix and as.data.frame is a bit silly, but required to have the right format

gr <- graph_from_edgelist(edgelist)

#### distance between each neighbouring cell is 250m since we defined this when we created the grid
...
whereispixels <- as.numeric(st_intersects(coords, hex_elev_umt))
dist3 <- distances(gr, whereispixels[1], whereispixels[-1])
