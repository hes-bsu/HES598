...
#enerating x number of RANDOM points in a circle
...
plot(p_cluster, main= "Simulated Points: Clustered Spacing")

#generating x number of REGULAR points (aka a repulsive point pattern) in a circle
set.seed(123)
p_regular <- rStrauss(beta = 2.9, gamma = 0.025, R = .5, W = disc10)
plot(p_regular, main="Simulated Points: Regular Spacing")
```
...
I'll also bring in and test sample data I've made in Google Earth (and then converted to a shapefile) of points where informal trails branch off of established trails and roads. These points are from an area within a 1 mile radius of one of the Mulford's milkvetch populations in the Owyhee Front.
```{r}
#Read in data
...
# The p value is <.01, and there appears to be evidence of uniform spacing. 
```
...
The other main package associated with the spatial analysis done here (mainly dealing with the sample data I brought in, and adding a projection to it) is sf. Sf is a workhorse spatial analysis package with extensive documentation. More information can be found about it here: https://www.rdocumentation.org/packages/sf/versions/0.9-2
```{r}

```

## Benchmarking
The analyses I'm doing are pretty straightforward and not too data heavy on the points I'm using right now. But that may not always be the case. I'll test using a Ripley's K vs using a quadrat test to see if one is faster. These tests tell me slightly different things, but if one is way faster than the other and all I need to know is if something is clustered or not, that could be useful to know if I ever try to run these tests on a huge dataset. 
...
## Final product
Graphs of the Ripley's K test show a solid black line, which indicates clustering when it deviates from zero, and more clusteing the further from zero it is. Along the x axis we see r, the radius of the spatial extent being tested. For the clustered simulation data, we see evidence of clustering, especially at a larger spatial extent. In the regular simulated data data, we see a largely flat line close to zero, indicating little clustering (as expected.) In the sample data, we see substatial clustering at most spatial scales. 
```{r}
plot1 <-plot(K_cluster_env.clustered, .-pi*r^2 ~ r)

plot2 <-plot(K_cluster_env.regular, .-pi*r^2 ~ r)
plot3 <-plot(K_cluster_env.sample, .-pi*r^2 ~ r)
```
