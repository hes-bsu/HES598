...
```{r applyintro}
#create data
test.matrix <- matrix(rnorm(200), 20, 10)##20 rows, 10 cols
dim(test.matrix)
# generate column means
apply(X=test.matrix, MARGIN = 2, mean) # MARGIN = 2 specifies columns
#generate row sums
...
lapply(tif_files$id, function(x) drive_download(as_id(x), overwrite = TRUE)) ## need to use function(x) syntax then list other args following to deal with functions with args>1

rstr.names <- list.files(pattern = "*.tif")
rstrs <- lapply(rstr.names, raster) ## lapply always produces a list
plot(rstrs[[1]])
```
...
rstrs$fun <- mean
rstrs$na.rm <- TRUE
## do.call requires (list, function) args; explicit about the function args -> great for rasters
mos <- do.call(mosaic, rstrs) ## more flexible than lapply -> mosaics each raster in the list; takes the mean of overlapping vals
plot(mos)
```
...
library(parallel)
detectCores()
## leave one core available for the OS
```
